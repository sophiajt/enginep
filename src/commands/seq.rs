use crate::commands::echo::RangeIterator;
...
use nu_source::{PrettyDebug, SpannedItem, Tag, Tagged, TaggedItem};
...
fn seq(args: CommandArgs, scope: &Scope) -> Result<ValueIterator, ShellError> {
    ...
    let start = match args.nth(0).unwrap() {
        Value {
            value: UntaggedValue::Primitive(x),
            tag,
        } => x,
        Value { tag, .. } => {
    ...
    let end = match args.nth(1).unwrap() {
        Value {
            value: UntaggedValue::Primitive(x),
            tag,
        } => x,
    ...
    Ok(Box::new(
        ...
        .map(|x| {
            UntaggedValue::string(format_leaf(&x).plain_string(100_000)).into_untagged_value()
        }),
    ))
