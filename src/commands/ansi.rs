use crate::{empty_value_iterator, CommandArgs, EvaluationContext, Example, ValueIterator};
...
Format: #
    ...
    fn run(&self, args: CommandArgs, scope: &Scope) -> Result<ValueIterator, ShellError> {
        ...
        let escape = args.get("escape");

        if let Some(e) = escape {
            let tag = e.tag.clone();
            if let Ok(e) = e.as_string() {
                let esc_vec: Vec<char> = e.chars().collect();
                if esc_vec[0] == '\\' {
