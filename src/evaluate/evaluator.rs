...
fn evaluate_reference(
    ...
    match name {
        "$nu" => crate::evaluate::variables::nu(scope, tag),
...
fn evaluate_invocation(
    ...
    let input = match scope.get_var("$it") {
        Some(it) => Box::new(std::iter::once(it)),
        None => empty_value_iterator(),
    };
    ...
    let output: Vec<_> = result.collect();
