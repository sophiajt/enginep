...
impl Scope {
    ...
    pub fn add_env(&self, env_vars: HashMap<String, String>) {
        if let Some(frame) = self.frames.lock().last_mut() {
            frame.env.extend(env_vars)
        }
    }

    pub fn add_env_to_base(&self, env_vars: HashMap<String, String>) {
...
impl ParserScope for Scope {
    ...
    fn add_definition(&self, block: Block) {
        if let Some(frame) = self.frames.lock().last_mut() {
            ...
            frame.commands.insert(name, command(block));
        }
    }
...
pub struct ScopeFrame {
    ...
    pub custom_commands: HashMap<String, Block>,
